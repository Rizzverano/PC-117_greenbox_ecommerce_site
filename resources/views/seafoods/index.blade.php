                ...
                <div class="card shadow-sm" style="border-radius: 15px; overflow: hidden;">
                    <div class="card-header py-3" style="background-color: #28a745; color: white;">
                        <div class="d-flex justify-content-between align-items-center">
                            <h2 class="h4 mb-0">Seafoods Category</h2>
                            <a href="{{ route('seafoods.create') }}" class="btn btn-light btn-sm" title="Add New Seafood" style="color: #28a745;">
                                            ...
                                            <td>
                                                <div class="d-flex flex-column gap-2">
                                                    <div class="d-flex gap-1">
                                                        <a href="{{ route('seafoods.show', $item->id) }}" title="View Seafood" class="btn btn-info btn-sm py-1 flex-grow-1">
                                                            <i class="fa fa-eye" aria-hidden="true"></i>
                                                        </a>
                                                        <a href="{{ route('seafoods.edit', $item->id) }}" title="Edit Seafood" class="btn btn-primary btn-sm py-1 flex-grow-1">
                                                            <i class="fa fa-pencil-alt" aria-hidden="true"></i>
                                                        </a>
                                                    </div>
                                                    <form method="POST" action="{{ route('seafoods.destroy', $item->id) }}" accept-charset="UTF-8" class="w-100">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-danger btn-sm py-1 w-100"
                                                            title="Delete Seafood"
